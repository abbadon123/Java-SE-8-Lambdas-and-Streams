...
public class Lesson3 {
  ...
  static int[][] computeLevenshtein(List<String> wordList, boolean parallel) {
    ...
    int[][] distances = new int[LIST_SIZE][LIST_SIZE];
      ...
      if(parallel) {
          IntStream.range(0, wordList.size()).parallel().forEach(
                  i -> IntStream.range(0, wordList.size()).parallel().forEach(
                          j -> distances[i][j] = Levenshtein.lev(wordList.get(i), wordList.get(j))
                  )
          );
      } else {
          IntStream.range(0, wordList.size()).sequential().forEach(
                  i -> IntStream.range(0, wordList.size()).sequential().forEach(
                          j -> distances[i][j] = Levenshtein.lev(wordList.get(i), wordList.get(j))
                  )
          );
      }
    ...
    static int[][] computeLevenshteinImperative(List<String> wordList) {
        final int LIST_SIZE = wordList.size();
        int[][] distances = new int[LIST_SIZE][LIST_SIZE];
  ...
  public static void main(String[] args) throws IOException {
    ...
    List<String> wordList = fullWordList.createList(15000);


//    measure("Parallel", () -> computeLevenshtein(wordList, true));
//    measure("imperative", () -> computeLevenshteinImperative(wordList));
//    measure("Sequential", () -> computeLevenshtein(wordList, false));

    measure("words Sequential", () -> processWords(wordList, false));
    measure("words Parallel", () -> processWords(wordList, true));
  }
